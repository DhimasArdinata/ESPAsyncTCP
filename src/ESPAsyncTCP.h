...
class ACErrorTracker {
 ...
 protected:
  ...
  size_t getConnectionId(void) const { return _connectionId; }
#endif
...
class AsyncClient {
 ...
 public:
  ...
  std::shared_ptr<ACErrorTracker> getACErrorTracker(void) {
...
class AsyncServer {
...
#ifdef DEBUG_MORE
  void incEventCount(size_t ee) {
    if (ee < EE_MAX) ++_event_count[ee];
  }
#endif
 ...
 public:
  ...
  int getEventCount(size_t ee) const {
    return (ee < EE_MAX) ? _event_count[ee] : 0;
  }
#endif
};
